...
const updateOrder = async function (req, res) {
    ...
    if (updatedOrder.status == "completed") {
        let orderComp = await cartModel.findOneAndUpdate({ userId: userId }, { $set: { items: [], totalItems: 0, totalPrice: 0 } }, { new: true })
        return res.status(200).send({ status: true, message: "Order status updated successfully", data: orderComp })
    }
